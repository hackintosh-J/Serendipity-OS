...
import { ModalType, ActiveAssetInstance } from '../../types';
import { ClockIcon, UploadIcon, TrashIcon } from '../../assets/icons';
import Modal from '../shared/Modal';
import Button from '../shared/Button';
import { storageService } from '../../services/storageService';

const InsightListItem: React.FC<{ asset: ActiveAssetInstance }> = ({ asset }) => {
    const { deleteArchivedInsight, restoreArchivedInsight } = useOS();
    const [imageUrl, setImageUrl] = useState<string | null>(null);

    useEffect(() => {
        let isMounted = true;
        const storageKey = asset.state.generated_image_storageKey;
        if (storageKey) {
            storageService.getItem<string>(storageKey).then(url => {
                if (isMounted && url) {
                    setImageUrl(url);
                }
            });
    ...
    }, [asset.state.generated_image_storageKey]);

    return (
        <li className="bg-secondary/50 p-4 rounded-lg shadow-sm">
...
const InsightHistoryModal: React.FC = () => {
    const { osState, setActiveModal } = useOS();
                ...
                {insightHistory.length === 0 ? (
                    ...
                    <ul className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                        {insightHistory.slice().reverse().map(asset => (
                            <InsightListItem key={asset.id} asset={asset} />
                        ))}
                    </ul>
                )}
            </div>
